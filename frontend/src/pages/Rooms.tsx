...
const Rooms: React.FC = () => {
  ...
  const fetchRooms = async () => {
    try {
      ...
      if (data.success) {
        // Filter rooms to only show manager's hotel rooms
        const managerHotels = await fetch('/api/hotels/manager/my-hotels', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const hotelData = await managerHotels.json();
        if (hotelData.success) {
          const hotelIds = hotelData.hotels.map((hotel: any) => hotel._id);
          const managerRooms = data.rooms.filter((room: any) => 
            hotelIds.includes(room.hotelId._id)
          );
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this room?')) {
      try {
        ...
        if (response.ok) {
          fetchRooms();
          fetchHotels(); // Refresh hotels to update room counts
        } else {
