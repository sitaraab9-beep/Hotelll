...
const Favorites: React.FC = () => {
  ...
  const removeFavorite = async (hotelId: string) => {
    ...
    try {
      ...
      const { toggleFavorite } = await import('../utils/mockData');
      toggleFavorite(user.id, hotelId, hotel?.name || '', hotel?.location || '');
      ...
      {favorites.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {favorites.map((favorite) => {
            const hotel = mockHotels.find(h => h._id === favorite.hotelId);
